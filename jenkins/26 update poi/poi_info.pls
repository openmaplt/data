...
r jsonb;
begin
  insert into places.log(t, l) values ('POI_INFO: ' || p_params::text, now());
  SELECT
    json_build_object(
      'extent', ARRAY[
  ...
  FROM (
    SELECT st_transform(ST_Expand(st_transform(geom, 3346), 150), 4326) AS expanded_box
     ...
     WHERE id = (p_params->>'id')::bigint
       and p_params->>'mapType' = 'places'
    union
    SELECT st_transform(ST_Expand(st_transform(way, 3346), 150), 4326) AS expanded_box
          ,'PUB' AS type
          ,id::text
          ,places.create_public_attr(p.*) AS properties
          ,st_transform(way, 4326) AS geom
      FROM public.poi p
     WHERE id = (p_params->>'id')::bigint
       and p_params->>'mapType' = 'craftbeer'
  ) AS subquery;

  if r is null then
    insert into places.log(t, l) values ('POI_INFO NOT FOUND FOR PARAMS: ' || p_params::text, now());
    return '{}'::jsonb;
  else
    return r;
  end if;
exception when others then
  insert into places.log(t, l) values ('POI_INFO ERROR: ' || sqlerrm || ' GOT PARAMS: ' || p_params, now());
